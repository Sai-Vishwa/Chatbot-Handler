...
mcpServer.tool("fetch_All_Marks", "Returns marks of all the students", {
...
mcpServer.tool("fetch_One_Mark", "Returns marks of one student with their roll number as input", {
    roll_no: z.number().describe("roll number of the student whose marks is to be fetched")
}, async ({ roll_no }) => {
    const resp = await fetchOneMarkFunction(roll_no);
    if (resp.isErrorResponse) {
        return {
            content: [
                {
...
mcpServer.tool("fetch_Marks_In_A_Range", "Returns marks of the students whose marks fall in the provided range", {
